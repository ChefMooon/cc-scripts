...
--- mooonOS/digOS/digOSUtil.lua
--- mooonOS/digOS/rednetUtil.lua
--- 
...
local function startupInventoryCheck()
    if digArgs.ignoreInventory == "true" then
        sendJobUpdate("Inventory Ignored.")
...
local function getRequiredRemainingFuel()
    local remainingLength = digArgs.length - layersMined
    local wFuel = 1
    if digArgs.width > 2 then
        wFuel = digArgs.width - 2
    end
    return ((wFuel * digArgs.height) * remainingLength) + layersMined + digArgs.length
end
...
local function getRequiredFuel()
    ...
    return ((wFuel * digArgs.height) * digArgs.length) + (digArgs.length * 2) -- maybe only length * 1
end
...
local function returnToStart()
    ...
    local torchOffset = digArgs.torch.torch == "true" and torchPlaced == true
    zPosReset()
    if torchOffset then
        moveUp()
    end
...
local function placeChest()
    local selectedSlot = turtle.getSelectedSlot()
    turtle.select(digArgs.chest.slot)
    turtle.place()
    turtle.select(selectedSlot)
end
...
local function tryEmptyInventory()
    ...
    elseif turtle.getItemCount(digArgs.chest.slot) > 0 then
        placeChest()
        if emptyAll() then
            halfSpin()
            sendJobUpdate("Inventory Emptied.")
...
if digArgs.torch.torch == "true" then
    removeAvailableInventorySlot(digArgs.torch.slot)
end
if digArgs.chest.chest == "true" then
...
if digArgs.command == "run" then
    ...
    while true do
        ...
        if startFuelCheck() then
            ...
            if digArgs.ignoreFuel == "true" then
                sendJobUpdate("Fuel Ignored.")
            else
