...
function view.init(frame, info, rednetInfo, progInfo, theme)
    ...
    components.homeTurtleID = components.homeTurtleIDFrame:addLabel():setText(tostring(info.idLabel))
    ...
    components.programNameLabel = components.programInfoFrame:addLabel():setText("Version:"..progInfo.version):setPosition(1,2):setSize("{parent.w-1}",1)
end
...
function view.toggleNetworkButtons(on, theme)
    if on then
        components.homeNetworkOffButton:setForeground(theme.networkFalse)
        components.homeNetworkOnButton:setForeground(theme.networkTrue)
    else
        components.homeNetworkOffButton:setForeground(theme.networkTrue)
        components.homeNetworkOnButton:setForeground(theme.networkFalse)
    end
end
